...
@GeneratedDsl
class IsCheckedOperation(
    @DefaultValue("30000.0")
    val timeout: Double = 30000.0
) : PlaywrightConfig<Locator.IsCheckedOptions>, LocatorOperation {
    @TransientDsl
    var result: Boolean = false

    override fun process(locator: Locator) {
        result = locator.isChecked(toPlaywright())
    }

    override fun toPlaywright(): Locator.IsCheckedOptions = Locator.IsCheckedOptions().also {
        it.timeout = timeout
    }
}
